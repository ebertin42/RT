...
void	color_changer(Uint32 *color, t_env *e, t_inter t)
{
...
{
	print_text(ft_strdup("Color"), s->font.color[1],
	s, &s->hud1.color_text);
	s->hud1.color_text.rect = init_sdl_rect(SIZE_X + COL4 + 80, WIN_Y / 2
			+ LINE + 100 + 10, 100, 20);
	print_text(ft_strdup("Reflection"), s->font.color[1],
	s, &s->hud1.reflex_text);
	s->hud1.reflex_text.rect = init_sdl_rect(SIZE_X + COL4 + 80, WIN_Y / 2
			+ LINE + 160 + 10, 110, 20);
	print_text(ft_strdup("Transparency"), s->font.color[1],
